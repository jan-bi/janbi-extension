...
팝업으로 열린 창은 부모 창과는 완전히 별개의 DOM 트리와 JavaScript 스코프를 갖습니다.<br>
...
<img src="https://i.ibb.co/NdXdM87s/iframe.png" alt="iframe" border="0"></a>
...
## 2. 선택된 요소의 텍스트, 이미지 변화를 어떻게 감지할까?

JANBI는 사용자의 지정한 요소의 '의미 있는 시각적 변화'를 감지하는 데 초점을 맞췄습니다.<br>

### 2.1 사용자가 선택한 요소는 서버에 저장됩니다.
...
### 2.2 변화 감지는 텍스트와 이미지 주소를 기준으로 합니다.
...
### 2.3 비교 결과는 변경 여부와 함께 이력에 저장됩니다.
...
이렇게 불필요한 알림을 줄이고 사용자에게 정확하고 의미 있는 변화만을 전달합니다.
...
### 3.2 CSS Selector는 직관적이지만 한계가 있습니다.
...
### 3.3 XPath 또한 구조 기반 접근이 가능하지만 한계가 있습니다.
...
### 3.4 XPath와 CSS Selector 혼합 방식을 선택했습니다.
...
CSR(Client-Side Rendering) 기반 웹사이트는 JavaScript 실행 후에야 실제 DOM이 구성되기 때문에 렌더링 완료 전에 요소를 탐색하면 선택이 실패합니다.

### 원인: Cheerio는 렌더링되지 않은 HTML만 처리할 수 있습니다.

Cheerio는 정적 HTML만 파싱할 수 있는 도구입니다. JavaScript로 구성되는 CSR 페이지의 실제 DOM은 반영하지 못합니다.

- HTML 구조는 존재하지만 데이터가 없음
- 요소 선택자에 해당하는 내용이 HTML에는 존재하지 않음

결과적으로 중요한 정보가 누락된 상태로 비교하거나 변경 감지를 하지 못하는 문제가 발생했습니다.
...
CSR 페이지는 요소가 늦게 나타나기 때문에 타이밍을 잘 맞추지 않으면 요소를 찾지 못하는 문제가 발생했습니다.<br>
Playwright를 사용해 렌더링된 페이지에서 요소를 찾으려고 해도 페이지가 완전히 그려지기 전에 탐색을 시도하면 해당 요소가 DOM에 존재하지 않아 탐지가 실패합니다.
...
`isVisible()`은 요소가 실제로 화면에 보이는지를 확인해주며 `display: none`, `opacity: 0`, `visibility: hidden` 등으로 가려진 요소는 감지 대상에서 제외됩니다.

### 결과: 실제로 보이는 요소만 안정적으로 탐지할 수 있게 되었습니다.
...
- 컴포넌트 기반 구조로 상태 변화가 잦은 UI를 효율적으로 구현할 수 있습니다.

### 2. Node.js + Express.js

- 비동기 I/O 구조로 스크래핑, 알림 처리에 적합합니다.
- NPM 생태계를 통해 다양한 기능을 빠르게 도입할 수 있습니다.
